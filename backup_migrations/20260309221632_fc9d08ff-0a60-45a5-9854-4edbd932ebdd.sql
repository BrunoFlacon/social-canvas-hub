...
-- Add profile_image_url to social_connections
ALTER TABLE public.social_connections ADD COLUMN IF NOT EXISTS profile_image_url text;
...
-- Update social_connections_safe view to include profile_image_url
DROP VIEW IF EXISTS public.social_connections_safe;
CREATE VIEW public.social_connections_safe WITH (security_invoker=on) AS
  SELECT id, user_id, platform, platform_user_id, page_name, page_id, is_connected, token_expires_at, profile_image_url, created_at, updated_at
