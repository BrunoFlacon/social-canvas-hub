-- ============================================================
-- FIX v4: Create messaging_members & add google_provider_token
-- Run this in Supabase SQL Editor
-- ============================================================

-- 1. Add google_provider_token to profiles (if not exists)
...
-- 3. Create messaging_audience_logs table (idempotent)
...
DROP POLICY IF EXISTS "Users manage own members" ON public.messaging_members;
CREATE POLICY "Users manage own members" ON public.messaging_members 
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users manage own audience logs" ON public.messaging_audience_logs;
CREATE POLICY "Users manage own audience logs" ON public.messaging_audience_logs 
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- 5. Unique constraints for upsert
ALTER TABLE public.messaging_members 
  DROP CONSTRAINT IF EXISTS messaging_members_unique_member;
ALTER TABLE public.messaging_members 
  ADD CONSTRAINT messaging_members_unique_member 
  UNIQUE (user_id, platform, channel_id, telegram_user_id);
