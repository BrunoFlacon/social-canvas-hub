...
import {
  ...
  Phone, MessageSquare, Calendar, Mail, NewsapiIcon, Image as ImageIcon, Link2, LogOut, Pencil, Laptop, Clock,
...
import { GoogleIcon, FacebookIcon, MetaIcon, Newspi} from "@/components/icons/SocialIcons";
  ...
  const syncSocialStats = async (platformId?: string) => {
    ...
    try {
      ...
      const invokeFn = async (fnName: string, bodyObj?: any) => {
        const { error: invErr } = await supabase.functions.invoke(`${fnName}?_cb=${Date.now()}`, {
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                  {[
                                    ...
                                    { name: 'People API', key: 'people_api_key', desc: 'Sincronização de Contatos' },
                                  ].map(svc => {
                                      ...
                                      <div key={svc.name} className={cn(
                                        ...
                                        <div className="flex items-center justify-between">
                                          ...
                                          <Button
                                            variant="ghost"
                                            size="sm"
                                            className={cn("h-6 px-2 text-[8px] font-bold uppercase", isActive ? "text-red-400 hover:text-red-500 hover:bg-red-500/10" : "text-primary hover:bg-primary/10")}
                                            onClick={() => {
                                              if (isActive) {
